...
Rules are carried out by an iRODS rule engine plugin---a built-in interpreter for the rule language being used. The rule engine governs the sequence of data management actions in your iRODS zone. The Catalog Provider and each Consumer server run an instance of the rule engine. Out of the box, the rule engine comes loaded with an array of basic actions (e.g., error reporting, login protocols) to get you up and running. As you begin to dig into iRODS, you can add rules of your own to tailor a data management program that works for you.
...
\hline
ubuntu & Linux & This is the user role on the Linux VM. You will be \texttt{ubuntu} throughout the day because you will always be using the Linux VM. However, you also may masquerade, so to speak, as some of the other roles. \\
...
irods & Linux & This is the Linux service account that is created by default when you install iRODS. It runs the iRODS software and owns all physical data objects. We will never masquerade as irods in the training and it is best not to do so in any case. \\
...
\newpage
The \texttt{setup\_irods.py} script will ask for information in four (and possibly five) sections.
...
\ugmbullet Catalog Service Role & Provider or Consumer, determines whether this server holds a connection to the metadata catalog (database). Select \texttt{Provider}.\\
\hline

\multicolumn{2}{|l|}{2. Database Connection (if installing a `Provider')} \\
\ugmbullet ODBC Driver & The driver on the server used to talk to the ODBC database layer. Select \texttt{1}. \\
\makecell[l]{\ugmbullet Database Server's \\ \hspace{8.5mm}Hostname or IP } & Enter \texttt{localhost}. \\
...
\ugmbullet Stored Passwords Salt & This protects the passwords stored in the database, making your installation unique.  For production use, don't leave this empty. \\
\hline
...
\ugmbullet zone\_key & A secret key used in server-to-server communication. \\
\ugmbullet negotiation\_key & A secret key used in server-to-server communication.  Must be exactly 32 bytes (characters). \\
...
On the same line, after the colon, enter: ec2-#-#-#-#.compute-1.amazonaws.com
...
Remember we set Alice's password as \texttt{passWORD}. We must log in with this first before we can change it.
...
Suppose Alice would like to copy her photos---or data objects---from a local directory to an iRODS collection. For this, use the \texttt{iput} command:
...
To empty the trash, we could use \texttt{irmtrash}. Using the \texttt{-f} command line option with irm will remove the data object permanently. It cannot be recovered if you use \texttt{-f}.
...
There are no replicas. So let's replicate \texttt{peanuts.jpg} to \texttt{newResc} using the \texttt{irepl} command:
...
Rule Engine & \vspace{-3mm}\makecell[l]{iRODS Rule Language \\ Python* \\ C++* \\ Javascript*} \\
...
\end{tabular}
\end{center}
\footnotesize\vspace{-5mm}\hspace{3mm}* External plugins. All other plugins listed are installed by default.
\normalsize
...
\begin{tabular}{ |l|p{10cm}| }
\hline
unixfilesystem & This type of storage resource communicates with a storage device through the standard POSIX interface \\
...
Policy Enforcement Points are special types of rules that invoke an interpreted rule script only when certain criteria are met within the iRODS Agent (i.e., an instance of a process that handles API requests), such as when a data object is successfully placed within iRODS management. A PEP may launch rules that reside in the default rulebase, known as \texttt{core.re}, and are referenced by the name of the PEP, such as \texttt{acPostProcForPut}, which we will cover later in this section.

\subsection{The \texttt{delay} Directive}

Delayed Execution Rules are those rules that invoke the \texttt{delay} keyword (i.e., reserved word), which places the rule script in the delayed execution queue rather than immediately executing the rule. Rules may be executed on a delay manually by using the delay directive with the \texttt{irule} command to place the rule in a periodic queue that is monitored by the iRODS rule server.
...
\begin{itemize}
 \item a packaged form such as an rpm (i.e., RedHat Linux's package management sytem), or
 \item a raw, shared object which may have been custom built.
\end{itemize}
...
Custom-built microservice plugins must be copied to \texttt{\$IRODS\_HOME/plugins/microservices}. If iRODS was installed via a run-in-place installation, \texttt{\$IRODS\_HOME} (a command line variable signifying the installation location) will be wherever the code was built. For a packaged installation of iRODS, \texttt{\$IRODS\_HOME}  will be \texttt{/var/lib/irods/}.
...
\begin{lstlisting}
$ sudo apt-get install irods-rule-engine-plugin-python python-exif
\end{lstlisting}
...
\begin{lstlisting}
$ iget ftp://ftp.renci.org/pub/irods/training/stickers.jpg
$ iput stickers.jpg
\end{lstlisting}
