...
launch_thumbnail_compute(
    ...
    *guid = "NULL"
    msiget_uuid(*guid)
    *dst_dir_name = "/tmp/" 
    *dst_dir_name = *dst_dir_name ++ *obj_name ++ "-" ++ *guid
...
writeLine( "serverLog", "XXXX - [*src_obj_path] [*sz] [*thumbnail_name] [*dst_obj_path]" )
        ...
        *sz_str = str(*sz)
        *docker_option = "-v " ++ *src_dir_name ++ ":/src -v " ++ *dst_dir_name 
        *docker_option = *docker_option ++ ":/dst -e SIZE=" ++ *sz_str
        *docker_option = *docker_option ++ " -e SOURCE_IMAGE=" ++ *src_file_name 
        *docker_option = *docker_option ++ " -e DESTINATION_IMAGE=" ++ *thumbnail_name
